...
<!--                    <li><a href="../bron/admin_zagruzka.php">Резерв под загрузку</a></li>-->
                    ...
                    <li><a href="admin_propusk_personal.php">Пропуски на персонал</a></li>
            ...
            <div class="col-lg-9 col-md-9 col-sm-9 col-xs-6 margin_fixed">
                <div class="main_text"><span>П</span>ропуски арендаторов на сотрудников</div>
                ...
                while ($row = mysqli_fetch_assoc($arendatorinfo)) {
                    ...
                    $personalinfo = mysqli_query($connect, 'SELECT ID_personal, personal.familiya, personal.imya, personal.otchestvo FROM personal, user WHERE personal.ID_user=user.ID_user and user.ID_user = "' . $row['ID_user'] . '"');
                    $i = 1;
                    ...
                    while ($row = mysqli_fetch_assoc($personalinfo)) {
                        ...
                        echo '<div class="car_block sotr spoiler">';
                        echo '<div class="row">';
                        echo '<div class="col-lg-7 col-md-7 col-sm-7 col-xs-7 row_name_main place_name">Сотрудник №' . $i . '</div>';
                        echo '<div class="col-lg-5 col-md-5 col-sm-5 col-xs-5 right">';
                        echo '<form method="post" action="admin_func.php"><button class="delete" style="margin-top: 10px; margin-right: 10px; margin-bottom: 0;" name="delete_pers" value="' . $row['ID_personal'] . '"><img src="../img/icons8-delete.png" /></button></form>';
                        echo '</div>';
                        echo '</div>';
                        echo '<div class="row car_row place_row">';
                        echo '<div class="col-lg-3 col-md-3 col-sm-1 col-xs-6 row_name">ФИО:</div>';
                        echo '<div class="col-lg-9 col-md-3 col-sm-1 col-xs-6 row_value">' . $row['familiya'] . ' ' . $row['imya'] . ' ' . $row['otchestvo'] . '</div>';
                        echo '</div>';
                        echo '</div>';
                        echo '</div>';
